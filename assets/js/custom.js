document.addEventListener("DOMContentLoaded", () => {
  ...
  // ─── Mobile Menu Toggle ──────────────────────────────────
  const mobileToggle = document.querySelector(".lp-header__mobile-toggle");
  const mobileMenu = document.querySelector(".lp-mobile-menu");
  const mobileOverlay = document.querySelector(".lp-mobile-overlay");
  const mobileClose = document.querySelector(".lp-mobile-menu__close");
  const mobileLinks = document.querySelectorAll(".lp-mobile-menu__link");
  const mobileCta = document.querySelector(".lp-mobile-menu__cta");
  const mobileItems = document.querySelectorAll(".lp-mobile-menu__item");
  ...
  function openMobileMenu() {
    ...
    mobileItems.forEach((item, index) => {
      item.style.transitionDelay = `${0.05 + index * 0.06}s`;
...
window.addEventListener("load", function () {
  ...
  document.querySelectorAll(".lp-custom-vimeo")?.forEach(function (videoCard) {
    videoCard.addEventListener("click", function () {
      ...
      const iframeWrapper = videoCard.querySelector(
        ".lp-custom-vimeo__iframe-wrp",
      );
  ...
  document
    .querySelectorAll(".lp-custom-youtube")
    ?.forEach(function (videoCard) {
      videoCard.addEventListener("click", function () {
        // If already playing, don't recreate the iframe
        if (videoCard.classList.contains("active")) return;
